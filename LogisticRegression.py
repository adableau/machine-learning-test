...
# 梯度上升法
# #
def gradientAscent(dataMat, labelMat, alpha):  #dataMat是2维Numpy数组,每列代表不同特征;每行代表不同样本;alpha步长
    m,n=shape(dataMat) #训练集个数#数据特征纬度
    theta = [0] * n
    iter = 1000
    while(iter):
        for i in range(m):
            hypothesis = sigmoid(computeDotProduct(dataMat[i], theta)) #行列转换,转至
            error = labelMat[i] - hypothesis
            gradient = computeTimesVect(dataMat[i], error) #某个向量的n倍
...
def colicTest(tainFileName, testFileName):
    ...
    #trainWeights = gradientAscent(trainingSet, trainingLabels, 0.001)
    ...
    for line in frTest.readlines():
        ...
        if int(classifyVector(array(lineArr), trainWeights))!= int(currLine[21]):
            errorCount += 1
    rightRate = 1.0 - (float(errorCount)/numTestVec)
...
colicTest('data/Training.txt', 'data/Test.txt')
